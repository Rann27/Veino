...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
    ...
    // Initialize from localStorage immediately for instant application
    const getInitialReaderSettings = (): ReaderSettings => {
        ...
        try {
            ...
            if (cachedData) {
                ...
                if (data.reader_settings) {
                    return data.reader_settings;
                }
            }
            
            // Try direct settings
            const savedSettings = localStorage.getItem('veinovel-reader-settings');
            if (savedSettings) {
                return JSON.parse(savedSettings);
            }
        } catch (e) {
            console.error('Error loading initial reader settings:', e);
        }
        ...
        const loadFromLocalStorage = () => {
            ...
            if (savedReaderSettings) {
                try {
                    ...
                    console.log('Loaded reader settings from localStorage:', parsed);
                    setReaderSettings(parsed);
                } catch (e) {
                    console.log('Failed to parse localStorage reader settings:', e);
                    // Invalid JSON, use defaults
                }
            } else {
                console.log('No reader settings found in localStorage');
            }
        };
    ...
    const updateReaderSettings = (settings: Partial<ReaderSettings>) => {
        ...
        console.log('Updating reader settings:', newSettings);
